...
export async function main(ns) {
    ...
    ns.tprint(`Starting infiltration script. Target: ${target}`);
    ...
    const portCrack = (server) => {
        ...
        for (const file of Object.keys(portManager)) {
            if (ns.fileExists(file, home)) {
                ...
                ns.print(`Running ${file} on ${server}`);
            }
        }
    };
    ...
    const scpVirus = async (server) => {
        ...
        ns.print(`Executing ${virus} on ${server} with ${maxThreads} threads targeting ${target}`);
    ...
    while (true) {
        const numCracks = getNumCracks();
        ns.print(`Number of cracks available: ${numCracks}`);
        const newTargets = getTargetServers();
        ns.print(`Current target servers: ${currentTarget.join(", ")}`);
        ns.print(`New target servers: ${newTargets.join(", ")}`);

        // Check if targets have changed or if the number of cracks has increased
        if (newTargets.length !== currentTarget.length || !newTargets.every((val, index) => val === currentTarget[index])) {
            ...
            previousNumCracks = numCracks; // Update the previous number of cracks after deployment
        }
